...
class User:
    ...
    def __init__(self, username, id_num, DOB, password,account):
         self.name = username
    ...
    def showObjectProperties():
        ...
        for j in range(0,len(User.usersList)):
            ...
            print(User.usersList[j].account)
	...
	while count < 3:
		...
		for user in User.usersList:
			...
			print("You are welcome {}".format(username)
        		curr_date = date.today()
        		print("Today's date:",curr_date)
        		selection=int(input("Select an Option of transaction:"))
	...
	print("\n Register here:")
	print("--------------")
	newUser = User(input("Please enter a username"),
			(functions.getUniqueNumber()), 
			(str(functions.grabDate())),
			(str(functions.generate_account()))
        tosave = []
                ...
                if(selector.find(" ") != -1):
                    ...
                    if( len(matches) > 0):
                        ...
                        if( int(toRemove)-1 < len(matches) and (matches[int(toRemove)-1] < len(User.usersList)) ):
                            print("\n\nDeleting User: ",User.usersList[int(toRemove)]) 
                        else:
